...
public function reviewManagement() {
    ...
    $reviews = DB::table('review')
        ...
        ->select(
            ...
            'customers.full_name',
            'customers.customer_profile_images as customer_image_raw',
            'sellers.seller_id',
        ...
        ->map(function ($review) {
            // Add computed fields
            $review->review_message = $review->comment;
            $review->customer_name = $review->customer_id ? $review->full_name : 'Former Customer';
            $review->customer_image = $review->customer_id ? $review->customer_image_raw : null;
            
            // Remove the temporary fields if needed
            unset($review->comment);
            unset($review->customer_image_raw);
    ...
    public function allAdminMessages()
{
    ...
    // Get all messages for this admin with seller info
    ...
    // Process in PHP to get last message time and unread counts
    $conversations = [];
    
    foreach ($messages as $message) {
        $sellerId = $message->seller_id;
        
        if (!isset($conversations[$sellerId])) {
            $conversations[$sellerId] = (object) [
                'seller_id' => $sellerId,
                'seller_name' => $message->seller_name,
                'seller_profile_img' => $message->seller_profile_img,
        ...
        if ($message->is_read == 0 && $message->sender_type == 'seller') {
            $conversations[$sellerId]->unread_count++;
        }
    }
    
    // Convert to array and sort by last_message_time
    $conversations = collect(array_values($conversations))
